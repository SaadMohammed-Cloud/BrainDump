...
const TAG_CLASS: any = { work: 'tag-work', ideas: 'tag-ideas', personal: 'tag-personal', meeting: 'tag-work', finance: 'tag-personal', health: 'tag-ideas', learning: 'tag-ideas' }
...
export default function Home() {
  ...
  const [messages, setMessages] = useState([{ id: '0', role: 'ai', text: "Hi! Just dump your thoughts and I'll organize them automatically. Try writing a quick note!" }])
  ...
  const organizeTimer = useRef<any>(null)
  ...
  async function autoOrganize(body: string, noteId: string) {
    ...
    try {
      const res = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          type: 'organize',
          body,
          allNotes: notes.filter(n => n.id !== noteId).slice(0, 10)
        })
      })
      const organized = await res.json()
      if (organized.title) {
        const newBody = organized.summary ? `AI_SUMMARY:${organized.summary}::END_SUMMARY\n\n${body}` : body
        await supabase.from('notes').update({
          title: organized.title,
          tags: organized.tags || ['personal'],
          body: newBody
        }).eq('id', noteId)
  ...
  function getSummary(body: string) {
    if (!body) return null
    const match = body.match(/^AI_SUMMARY:(.+?)::END_SUMMARY/s)
    return match ? match[1].trim() : null
  }

  function getUserContent(body: string) {
    if (!body) return ''
    return body.replace(/^AI_SUMMARY:.+?::END_SUMMARY\n\n/s, '')
  }
  ...
  const related = activeNote ? notes.filter(n =>
    n.id !== activeId && n.tags?.some((t: string) => activeNote.tags?.includes(t))
  ).slice(0, 3) : []
  ...
  async function handleEdit(field: string, value: string) {
    ...
    await supabase.from('notes').update({ [field]: value }).eq('id', currentId)
    setSaveStatus('saved')
    if (field === 'body' && value.length > 20) {
      clearTimeout(organizeTimer.current)
      organizeTimer.current = setTimeout(() => {
        autoOrganize(value, currentId)
      }, 3000)
    }
  }
  ...
  async function sendChat(text?: string) {
    ...
    try {
      const res = await fetch('/api/chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ message: msg, notes, events: EVENTS })
      })
      const data = await res.json()
      setMessages(prev => [...prev, { id: Date.now().toString(), role: 'ai', text: data.text || 'Something went wrong.' }])
    } catch {
      setMessages(prev => [...prev, { id: Date.now().toString(), role: 'ai', text: 'Connection error.' }])
    }
    setTyping(false)
  }
        ...
        <div className="main">
          <div className="topbar">
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              {organizing && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: '#534AB7', background: '#EEEDFE', padding: '4px 10px', borderRadius: 20 }}>
                  <i className="ti ti-sparkles" style={{ fontSize: 13 }} /> AI organizing...
                </div>
              )}
                  ...
                  <div className="notes-scroll">
                    {filtered.map(note => (
                      <div key={note.id} className={`note-item ${note.id === activeId ? 'active' : ''}`} onClick={() => setActiveId(note.id)}>
                        ...
                        <div className="note-item-preview">{getUserContent(note.body).replace(/\n/g, ' ').substring(0, 60)}...</div>
                        <div className="note-item-meta">
                          {note.tags?.map((t: string) => <span key={t} className={`tag ${TAG_CLASS[t] || 'tag-personal'}`}>{t}</span>)}
                ...
                <div className="editor">
                  <div className="editor-toolbar">
                    ...
                    <div className="save-indicator">
                      {organizing
                        ? <><i className="ti ti-sparkles" style={{ fontSize: 12, color: '#534AB7' }} /> AI organizing...</>
                        : saveStatus === 'saving'
                    ...
                    {activeNote ? (
                      <>
                        <input
                          ...
                          placeholder="Start typing your note below — AI will title it for you..."
                        ...
                        <div className="editor-meta">
                          ...
                          {activeNote.tags?.map((t: string) => <span key={t} className={`tag ${TAG_CLASS[t] || 'tag-personal'}`} style={{ marginLeft: 4 }}>{t}</span>)}
                        </div>

                        {getSummary(activeNote.body) && (
                          <div style={{
                            background: '#EEEDFE',
                            ...
                            <span style={{ fontSize: 13, color: '#3C3489', lineHeight: 1.6 }}>
                              {getSummary(activeNote.body)}
                            </span>
                          </div>
                        )}

                        <div style={{ fontSize: 10, color: '#bbb', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 5 }}>
                          <i className="ti ti-pencil" style={{ fontSize: 11 }} /> Your note
                        </div>

                        <textarea
                          key={activeNote.id + '-b'}
                          className="editor-body"
                          defaultValue={getUserContent(activeNote.body)}
                          placeholder="Just dump your thoughts here... AI will organize it in 3 seconds ✨"
                          ...
                          onChange={e => {
                            ...
                            clearTimeout(organizeTimer.current)
                            organizeTimer.current = setTimeout(() => {
                              if (activeIdRef.current) autoOrganize(e.target.value, activeIdRef.current)
                            }, 3000)
                          }}
                        />
                        {related.length > 0 && (
                      ...
                      <div style={{ textAlign: 'center', color: '#bbb', marginTop: '4rem', fontSize: 14 }}>
                        <i className="ti ti-notebook" style={{ fontSize: 28, display: 'block', marginBottom: 8 }} />
            ...
            {view === 'search' && (
              <div className="search-panel">
                ...
                {filter && filtered.map(note => {
                  const userContent = getUserContent(note.body)
                  const idx = userContent?.toLowerCase().indexOf(filter.toLowerCase())
                  const snippet = (idx > -1 ? userContent.substring(Math.max(0, idx - 40), idx + 80) : userContent?.substring(0, 80) || '').replace(/\n/g, ' ')
                  return (
